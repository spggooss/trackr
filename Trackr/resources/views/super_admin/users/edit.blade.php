        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                    ...
                    <form method="POST" action="{{ route('super-admin.users.update', ['userId' => $user->id]) }}">
                        @csrf
                        <div class="mt-4">
                            <label for="name"
                                   class="block font-medium text-gray-700">{{ __('admin.users.index.table.name') }}</label>
                        ...
                        <div class="mt-4">
                            <label for="email"
                                   class="block font-medium text-gray-700">{{ __('admin.users.index.table.email') }}</label>
                        ...
                        <div class="mt-4">
                            <label for="webshop_id"
                                   class="block font-medium text-gray-700">{{ __('admin.users.index.table.webshop') }}</label>
                            <select name="webshop_id" id="webshop_id" class="form-select mt-1 block w-full">
                                @foreach ($webshops as $webshop)
                                    <option value="{{ $webshop->id }}"
                                            @if($user->webshop_id === $webshop->id) selected @endif>
                                        {{ $webshop->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mt-4">
                            <label for="role"
                                   class="block font-medium text-gray-700">{{ __('admin.users.index.table.role') }}</label>
                            <select name="role" id="role" class="form-select mt-1 block w-full">
                                @foreach (\App\Models\User\UserRole::getAll() as $role)
                                    <option
                                        value="{{ \App\Models\User\UserRole::create($role)->getValue() }}"
